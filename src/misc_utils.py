...
def hash_multi_batch(batch,weights,hparams):
    lengths=0
    for b,w in zip(batch,weights):
        for i in range(len(b)):
            b[i]=[abs(hash('key_'+str(i)+' value_'+str(x)))% hparams.multi_hash_num for x in b[i].split()] 
            lengths=max(lengths,len(b[i]))
            if len(b[i])==0:
                b[i]=[abs(hash('key_'+str(i)+' value_'+str('<pad>')))% hparams.multi_hash_num]
                w[i]=[1]
    batch_t=np.zeros((len(batch),len(hparams.multi_features),min(50,lengths)))+ abs(hash('key_'+str(i)+' value_'+str('<pad>')))% hparams.multi_hash_num
    weights_t=np.zeros((len(batch),len(hparams.multi_features),min(50,lengths)))

    
    for i in range(len(batch)):
        for j in range(len(batch[i])):
            for k in range(min(50,len(batch[i][j]))):
                batch_t[i,j,k]=batch[i][j][k]
                weights_t[i,j,k]=weights[i][j][k]
